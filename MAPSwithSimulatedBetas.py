...
def simulation(seed,p,n,k,sig_x,sig_z,mu_beta,sig_beta,ro,s1,s2):
    ...
    beta1,beta2,Y1,Y2,Y= MgenerateY(n1,n2,sig_z,mu_beta,sig_beta,p,fmrng,ro,'sim',s1,s2)
    ...
    ho1=multi_anchor_points_shrinkage(p,n1,k,eigen_result2,b,b,'beta_ordered')
    ...
    To1=tracking_error(K_actual, K_estimated3, ho1, b, beta2, q, sig_x, sig_z)
    ...
    result2['Beta Ordered']=diffNorm(ho1,b)
...
if __name__ == "__main__":
    ...
    for i in range(6):
        ...
        for seed in seed_list:
                print(i,seed)
                r1,r2 = simulation(seed,p,n,k,sig_x,sig_z,mu_beta,sig_beta,ro,1,1)
                result1.append(r1)
                result2.append(r2)
    
        result_df1 = pd.DataFrame(result1)
        result_df1 = result_df1.drop(['k','p'], axis=1)
        g = (1 + np.sqrt(5))/2
        f,ax = plt.subplots( figsize=(8,8/g))
        ax = sns.boxplot(data=result_df1)
        ax.set(ylim=(0,8))
        ax.set(xlabel="rho="+str(ro)+", p="+str(p), ylabel = "Tracking Error x p")
        plt.savefig("Expected Tracking Error, rho="+str(ro)+".png") 
                
        result_df2 = pd.DataFrame(result2)
        result_df2 = result_df2.drop(['k','p'], axis=1)
        g = (1 + np.sqrt(5))/2
        f,ax = plt.subplots( figsize=(8,8/g))
        ax = sns.boxplot(data=result_df2)
        ax.set(ylim=(0,1))
        ax.set(xlabel="rho="+str(ro)+", p="+str(p), ylabel = "L_2 Error")
        plt.savefig("Expected L_2 Error, rho="+str(ro)+".png") 
