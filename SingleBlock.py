...
def betas(mu_beta, sig_beta, fmrng, ro, model,*args):
    ...
    The possible values for the model parameter are "data1", "data2" and "sim":
    % When model is set to "data1" the function draws beta1 from the data and set beta2 equal to beta1. 
    Essentially, in this case, the function returns two identical beta's drawn from the data.
    ...
    *args    : it is a list of two parameters that encodes the time window numbers we are getting our betas from.
    ...
    # We create the single block beta from the csv file obtained from S&P 500 data.
    
    
    if model=='data1':
        
        name="b"+str(int(s[0]))+".csv"
        df=pd.read_csv(name)
        ...
        a=p/a   
        beta1=beta1*a
        beta2=beta1  # Setting beta2=beta1 will create a single block.
...
def simulation(seed,p,n,k,sig_x,sig_z,mu_beta,sig_beta,ro,s1,s2):
    ...
    hs2=multi_anchor_points_shrinkage(p,n,k,eigen_result3,h3,b, 'sector_stressed') #sector seperation on double block of data. 
    result1 = {}
    result1['k'] = k
    result1['p'] = p
    ...
    phi_squared3=eigen_result3['phi_square']
    K_actual=(sig_z*sig_z)/(sig_x*sig_x*vecNorm(beta2))
    K_estimated3=(n/p)*(1/phi_squared3-1)
    ...
    Ts2=tracking_error(K_actual, K_estimated3, hs2, b, beta2, q, sig_x, sig_z)
    
    
    result1['PCA']=p*Tp2
    result1['GPS']=p*Tg2
    result1['Sector Seperated']=p*Ts2
    ...
    result2['k'] = k
    result2['p'] = p
    result2['PCA']= diffNorm(h3,b)
    result2['GPS']=diffNorm(hg2,b)
    result2['Sector Seperated']=diffNorm(hs2,b)
    ...
    result3['k'] = k
    result3['p'] = p
    result3['PCA']=p*OptBias(h3,b,p)
    result3['GPS']=p*OptBias(hg2,b,p)
    result3['Sector Seperated'] = p*OptBias(hs2,b,p)
...
if __name__ == "__main__":
    ...
    for j in range(1,24):
        ...
        for seed in seed_list:
                print(j,seed)
                r1,r2,r3=simulation(seed,p,n,k,sig_x,sig_z,mu_beta,sig_beta,ro,j,j)
                result1.append(r1)
                result2.append(r2)
                result3.append(r3)
        ...
        ETracking_errors['PCA'] =  result_df1['PCA'].mean() 
        ETracking_errors['GPS'] =  result_df1['GPS'].mean()
        ETracking_errors['Sector Seperated'] =  result_df1['Sector Seperated'].mean()
        ETracking_errors['Beta Ordered'] =  result_df1['Beta Ordered'].mean()
        ETracking_errors_list.append(ETracking_errors)
        El2errors['PCA'] =  result_df2['PCA'].mean()
        El2errors['GPS'] =  result_df2['GPS'].mean()
        El2errors['Sector Seperated'] =  result_df2['Sector Seperated'].mean()
        El2errors['Beta Ordered'] =  result_df2['Beta Ordered'].mean()
        El2errors_list.append(El2errors)
        Opt_Bias['PCA'] =  result_df3['PCA'].mean() 
        Opt_Bias['GPS'] =  result_df3['GPS'].mean()
        Opt_Bias['Sector Seperated'] =  result_df3['Sector Seperated'].mean()
        Opt_Bias['Beta Ordered'] =  result_df3['Beta Ordered'].mean()
    ...
    plt.savefig('SingleBlock_Tracking')
    plt.close()
    ...
    plt.savefig('SingleBlock_Opt_Bias')
    plt.close()
